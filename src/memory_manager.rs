...
impl MemoryManager {
    ...
    pub fn get_data(&self, region: usize, start: usize) -> *mut u8 {
        let state = self.inner.lock().unwrap();
        if let Some(region) = state.regions.get(region) {
            unsafe { region.ptr.add(start) }
        } else {
            panic!("Invalid region ID");
        }
    }
...
impl MemoryBlock {
    ...
    pub fn get_data_ptr(
        ...
        let state = self.manager.upgrade().ok_or("MemoryManager dropped")?;
        let region = state.get_data(region, start + offset);
        Ok(region)
    }
}
